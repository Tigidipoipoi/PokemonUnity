...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (accessedMapSettings != null)
        {
            ...
            {
                namez += GameController.Instance.PokemonDb.GetPokemonSpeciesByGameId(encounterList[i].ID.ToString()).Name + ", ";
            }
        ...
        while (true)
        {
            ...
            if (canInput)
            {
                ...
                if (Input.GetButton("Start"))
                {
                    ...
                    {
                        if (setCheckBusyWith(PKUScene.main.Pause.gameObject))
                        {
                            animPause = true;
                            PKUScene.main.Pause.gameObject.SetActive(true);
                            StartCoroutine(PKUScene.main.Pause.control());
                            while (PKUScene.main.Pause.gameObject.activeSelf)
                            {
                                yield return null;
                            }
                            unsetCheckBusyWith(PKUScene.main.Pause.gameObject);
                        }
                    }
                }
    ...
    public IEnumerator wildEncounter(WildPokemonInitialiser.Location encounterLocation)
    {
        if (accessedMapSettings.getEncounterList(encounterLocation).Length > 0)
        {
            ...
            {
                if (setCheckBusyWith(PKUScene.main.Battle.gameObject))
                {
                    BgmHandler.main.PlayOverlay(PKUScene.main.Battle.defaultWildBGM,
                        PKUScene.main.Battle.defaultWildBGMLoopStart);
                    ...
                    StartCoroutine(PKUScene.main.Battle.control(accessedMapSettings.getRandomEncounter(encounterLocation)));
                    ...
                    unsetCheckBusyWith(PKUScene.main.Battle.gameObject);
                }
            }
        }
    }
