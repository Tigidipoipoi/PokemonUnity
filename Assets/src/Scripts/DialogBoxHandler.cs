...
public class DialogBoxHandler : MonoBehaviour
{
    ...
    public void drawDialogBox(int lines)
    {
        ...
        DialogBoxTexture.pixelInset = new Rect(DialogBoxTexture.pixelInset.x, DialogBoxTexture.pixelInset.y, DialogBoxTexture.pixelInset.width, Mathf.Round((float)lines * 14f) + 16f);
        DialogBoxBorder.pixelInset = new Rect(DialogBoxBorder.pixelInset.x, DialogBoxBorder.pixelInset.y, DialogBoxBorder.pixelInset.width, Mathf.Round((float)lines * 14f) + 16f);
        DialogBoxText.pixelOffset = new Vector2(DialogBoxText.pixelOffset.x, 7f + Mathf.Round((float)lines * 14f));
        DialogBoxTextShadow.pixelOffset = new Vector2(DialogBoxTextShadow.pixelOffset.x, 6f + Mathf.Round((float)lines * 14f));
    }
    ...
    public void drawChoiceBox(string[] choices)
    {
        ...
        ChoiceBoxSelect.pixelInset = new Rect(342 - defaultChoiceWidth + 7, ChoiceBoxTexture.pixelInset.y + 9f + (14f * (choices.Length - 1)), ChoiceBoxSelect.pixelInset.width, ChoiceBoxSelect.pixelInset.height);
        ChoiceBoxText.pixelOffset = new Vector2(342 - defaultChoiceWidth + 21, ChoiceBoxTexture.pixelInset.y + 13f);
        ChoiceBoxTextShadow.pixelOffset = new Vector2(342 - defaultChoiceWidth + 22, ChoiceBoxTexture.pixelInset.y + 12f);
        ChoiceBoxText.text = "";
    ...
    public void drawChoiceBoxWidth(string[] choices, int width)
    {
        ChoiceBox.SetActive(true);
        ChoiceBoxTexture.texture = Resources.Load<Texture>("Frame/choice" + PlayerPrefs.GetInt("frameStyle"));
        ChoiceBoxTexture.pixelInset = new Rect(342 - width - 1, defaultChoiceY, width, 30f + (14f * (choices.Length - 1)));
        ChoiceBoxSelect.pixelInset = new Rect(342 - width + 7, ChoiceBoxTexture.pixelInset.y + 9f + (14f * (choices.Length - 1)), ChoiceBoxSelect.pixelInset.width, ChoiceBoxSelect.pixelInset.height);
    ...
    public IEnumerator scrollText()
    {
        ...
        while (DialogBoxText.pixelOffset.y < textPosDestination)
        {
            ...
            { //if text is about to spill over the top
                ...
                DialogBoxText.text = textMod[1] + "\n" + textMod[2];        //this involves splitting the string by it's line breaks
                DialogBoxTextShadow.text = DialogBoxText.text;
                ...
                DialogBoxTextShadow.pixelOffset = new Vector2(DialogBoxTextShadow.pixelOffset.x, DialogBoxTextShadow.pixelOffset.y - 14f);
            }
            DialogBoxText.pixelOffset = new Vector2(DialogBoxText.pixelOffset.x, DialogBoxText.pixelOffset.y + 1f);
            DialogBoxTextShadow.pixelOffset = new Vector2(DialogBoxTextShadow.pixelOffset.x, DialogBoxTextShadow.pixelOffset.y + 1f);
    ...
    { //No parametres means simply Yes/No
        ...
        while (!selected)
        {
            ...
            else if (Input.GetButtonDown("Back"))
            {
                ...
                {
                    chosenIndex -= 1;
                    ChoiceBoxSelect.pixelInset = new Rect(ChoiceBoxSelect.pixelInset.x, ChoiceBoxSelect.pixelInset.y - 14f, ChoiceBoxSelect.pixelInset.width, ChoiceBoxSelect.pixelInset.height);
                }
                SfxHandler.Play(selectClip);
