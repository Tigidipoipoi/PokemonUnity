...
public class PCHandler : MonoBehaviour
{
    ...
    private void updateSelectedInfo(OwnedPokemon selectedPokemon)
    {
        if (!carrying)
        {
    ...
    private void updateSelectedInfoOverride(OwnedPokemon selectedPokemon)
    {
        if (carrying)
        {
            carrying = false;
            ...
            if (currentPosition < 3)
            {
                ...
                else if (Input.GetButton("Select"))
                {
                    ...
                    else if (chosenIndex == 1)
                    {
                        ...
                        StartCoroutine(PKUScene.main.Typing.control(8, currentBoxHeader.text, PokemonGender.NONE,
                        ...
                        {
                            SaveData.currentSave.PC.boxName[currentBoxID] = PKUScene.main.Typing.typedString;
                        }
                        updateBoxesAndParty();
                        ...
                        if (cursorMode == CursorMode.Standard)
                        {
                            if (!carrying)
                            {
                                ...
                                else if (chosenIndex == 4)
                                {
                                    ...
                                    StartCoroutine(
                                        PKUScene.main.Summary.control(SaveData.currentSave.PC.boxes[currentBoxID],
                                            currentPosition - 3));
                            ...
                            if (!carrying)
                            {
                                ...
                                while (chosenIndex != 0)
                                {
                                    ...
                                    else if (chosenIndex == 4)
                                    {
                                        ...
                                        //Set SceneSummary to be active so that it appears
                                        PKUScene.main.Summary.gameObject.SetActive(true);
                                        StartCoroutine(PKUScene.main.Summary.control(SaveData.currentSave.PC.boxes[0],
                                            currentPosition - 33));
                                        //Start an empty loop that will only stop when ScenePC is no longer active (is closed)
                                        while (PKUScene.main.Summary.gameObject.activeSelf)
                                        {
                                    ...
                                    {
                                        //ITEM
                                        OwnedPokemon currentPokemon = SaveData.currentSave.PC.boxes[0][currentPosition - 33];
